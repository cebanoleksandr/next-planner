...
interface IProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  color?: 'ghost' | 'red' | 'green' | 'gray' | 'primary' | 'yellow';
  href?: string;
}
...
const Tag: FC<IProps> = ({ children, className, size = 'small', color = 'ghost', href, ...rest }) => {
  ...
  return (
    <div
      className={`${className} inline-block box-border mr-1 rounded-2xl ${sizeClasses[size]} ${colorClasses[color]} ${href ? 'cursor-pointer' : ''}`} 
      {...rest}
